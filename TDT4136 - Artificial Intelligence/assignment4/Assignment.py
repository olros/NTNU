...
class CSP:
    ...
    def __is_complete(self, assignment: dict[str, list[str]]) -> bool:
        """Checks if all the variables have only one possible left and therefore is complete"""
        return len([x for x in assignment.values() if len(x) > 1]) == 0
    ...
    def select_unassigned_variable(self, assignment: dict[str, list[str]]) -> str:
        ...
        """
        import random
        # Selects a random variable from all unassigned variables
        unassigned_variables = [x for x in assignment.items() if len(x[1]) > 1]
        return random.choice(unassigned_variables)[0]


    def inference(self, assignment: dict[str, list[str]], queue: list[tuple]) -> bool:
        ...
        while queue: # While the queue is not empty
            i, j = queue.pop(0) # Get the first item in the queue
            if self.revise(assignment, i, j):
                if not assignment[i]:
                    return False
                for neighbor, _ in self.get_all_neighboring_arcs(i):
                    if neighbor != j: # Don't append the currently popped queue-item
                        queue.append((neighbor, i))

        return True

    def revise(self, assignment: dict[str, list[str]], i: int, j: int) -> bool:
        ...
        for x in assignment[i]:
            ...
            if not [(x, y) for y in assignment[j] if (x, y) in self.constraints[i][j]]:
...
def main():
    tasks =['easy', 'medium', 'hard', 'veryhard']
    selected_task = ''
    while selected_task not in tasks:
        selected_task = input(f'Chose a task ({"|".join(tasks)}): ')
    csp = create_sudoku_csp(f'{selected_task}.txt')
    solution = csp.backtracking_search()
    if solution:
        print_sudoku_solution(solution)

        print(f'Backtrack called: {csp.backtrack_called}')
        print(f'Backtrack failures: {csp.backtrack_failures}')
    else:
        print('Failure - Could not find a solution')
